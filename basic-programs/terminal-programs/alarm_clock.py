...
# hide pygame welcome message and runtime warning
...
def main():
    """Prompt the user to set an alarm and start the timer."""
    try:
...
def set_timer(alarm_time):
    ...
    Continuously checks the current time every second and triggers
    an alarm sound when the current time reaches or passes the set time.
    """
    is_running = True
